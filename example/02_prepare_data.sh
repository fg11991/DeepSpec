...
# Data preparation for DSpark training on 8x Ascend 910B/910C.
# Three stages; `stages` selects which run (default all):
#   stages=3  train_data_path=/path/to/yours.jsonl   # own JSONL with usable
#             # answers: build the cache directly (subsample with `head -n N`)
...
# Stage-3 knobs. data_max_length overrides data.max_length for the cache; the
# training sequence length is fixed by the cache, so this is the only place to
